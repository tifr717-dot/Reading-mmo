(()=>{
  ...
  style.textContent=`
    ...
    /* The approved top crop already contains the exact Journal title artwork.
       Keep semantic heading text accessible without drawing a duplicate over it. */
  ...
  function enhance(){
    ...
    if(!shell.querySelector('.v51043-top-art')){
      ...
      art.src='./journal-mockup-top-exact-v1.webp?v=51043j';
      art.alt='';
    ...
    if(toolbar&&!toolbar.querySelector('.v51043-selector-art')){
      ...
      plate.src='./journal-mockup-selector-strip-blank-v1.webp?v=51043j';
      plate.alt='';
    ...
    if(label){
      ...
      label.textContent=select?.value==='all'?'ARCHIVE VIEW':'CURRENT BOOK';
    }

    const select=shell.querySelector('#v51034BookFilter');
    if(select){
      select.setAttribute('aria-label',select.value==='all'?'Archive view':'Current book');
