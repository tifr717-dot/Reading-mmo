...
function style(){
  ...
  s.textContent=
    ...
    "#v51045LevelOnlyHome{position:relative;width:100%;height:calc(100dvh - 64px - env(safe-area-inset-bottom));margin:0 auto;background:#160907;overflow:hidden;opacity:0}"+
    ...
    ".v51045-canvas{position:absolute;top:0;left:50%;width:max(100%,calc((100dvh - 64px - env(safe-area-inset-bottom)) * 2 / 3));aspect-ratio:2/3;transform:translateX(-50%);transform-origin:top center}"+
...
function mountProgressEditor(){
  ...
  if(!ring||!pct||!pageCurrent||!pageGoal)return;
  ...
  const groups=[
    ...
    {label:'PAGES',rows:[
      ['pageCurrentX','Current X',0,55,.1],['pageGoalX','Goal X',0,55,.1],
      ['pagesY','Pages Y',0,95,.1],['pagesSize','Font size',.5,6,.05]
    ]}
  ];
  ...
  const apply=()=>{
    ...
    if(out)out.textContent=
      ...
      'PAGES currentX:'+state.pageCurrentX.toFixed(2)+'%; goalX:'+state.pageGoalX.toFixed(2)+'%; top:'+state.pagesY.toFixed(2)+'%; font:'+state.pagesSize.toFixed(2)+'vw;\n'+
